...
public class BattleMap {
	...
	JButton[][] grid; //names the grid of buttons
		...
		ImageIcon water = new ImageIcon(new ImageIcon("water.png").
	...
	public void buttonMatrix(int width, int length){ 
	   ...
	   JTextField filenamesaved = new JTextField();
	   JLabel mylabel = new JLabel();
	   mylabel.setText("prova label");
	   newPan.setLayout(new BorderLayout());
	   newPan.add(save, BorderLayout.PAGE_START);
	   newPan.add(filenamesaved);
	   mylabel.setText("Inserire il nome del file da salvare (NON inserire .clp) --- Valore di default: mapEnvironment.clp");
	   Border border = mylabel.getBorder();
		Border margin = new EmptyBorder(10,10,10,10);
		mylabel.setBorder(new CompoundBorder(border, margin));
	   newPan.add(mylabel, BorderLayout.PAGE_END);
	   

	   save.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
               System.out.println("SAVING...");
			   String nameOfFile = !filenamesaved.getText().equals("") ? filenamesaved.getText() : "mapEnvironment";
			   System.out.println("Creating fine named: " + nameOfFile + ".clp");
				saveMap(nameOfFile);
			}});
	   c.add(newPan);
	}
        ...
        private void updateButton(int x, int y, String s){
			...
			ImageIcon ico = new ImageIcon(new ImageIcon(s+".png").
			getImage().getScaledInstance(45, 45, Image.SCALE_DEFAULT));
	...
	private void saveMap(String nameOfFile){

		try{
		      File file = new File(nameOfFile + ".clp");
