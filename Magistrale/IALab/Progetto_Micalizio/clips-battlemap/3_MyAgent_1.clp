...
;; ##### Identifico le guess-sure che ho fatto
(deftemplate guess-sure
  (slot x)
  (slot y)
)

;; ##### Identifico le fired che ho fatto, MA solo quelle vicine alle guess-sure
(deftemplate fired-gs
  (slot x)
  (slot y)
)
...
(defrule init-k-left-per-row (declare (salience 30))
...
(defrule init-k-left-per-col (declare (salience 30))
...
(defrule init-row-idx (declare (salience 30))
  (k-per-row (row ?x))
...
(defrule init-col-idx (declare (salience 30))
  (k-per-col (col ?y))
...
(defrule assert-dec-k-left-fire (declare (salience 20))
...
(defrule dec-k-left (declare (salience 20))
  ?dec <- (dec (x ?x) (y ?y))
...
(defrule assert-guess-fire-ok (declare (salience 20))
 (k-cell (x ?x) (y ?y) (content ?c&:(neq ?c water))) 
=>
  (assert (guess (x ?x) (y ?y)))
)

(defrule guess-fire-ok (declare (salience 20))
...
;; #### Rimuovo la guess-sure nel momento in cui ho fatto una max-fire-near-guess-sure e NON ho trovato acqua
(defrule fired-because-guess-sure (declare (salience 20))
  (fired-gs (x ?x) (y ?y))
  (k-cell (x ?x) (y ?y) (content ?c&:(neq ?c water))) 
  (or
    ?gs <- (guess-sure (x ?x1&:(eq ?x1 ?x)) (y ?y1&:(eq ?y1 (- ?y 1))))
    ?gs <- (guess-sure (x ?x2&:(eq ?x2 ?x)) (y ?y2&:(eq ?y2 (+ ?y 1))))
    ?gs <- (guess-sure (x ?x3&:(eq ?x3 (- ?x 1))) (y ?y3&:(eq ?y3 ?y )))
    ?gs <- (guess-sure (x ?x4&:(eq ?x4 (+ ?x 1))) (y ?y4&:(eq ?y4 ?y )))
  )
  => 
  (retract ?gs)
)
...
;; #####  Effettuo una fire nei punti limitrofi alle guess-sure che ho fatto. Tra tutti i punti vicini 
;;        alle guess-sure che ho fatto, prendo quella che è più promettente.
(defrule max-fire-near-guess-sure (declare (salience 10))
  (status (step ?s) (currently running))
  (moves (fires ?nf &:(> ?nf 0)))
  (row-idx (xs $? ?x $?))
  (col-idx (ys $? ?y $?))
  (not (k-cell (x ?x) (y ?y)))
  (k-left-per-row (row ?x) (num ?nx&:(neq ?nx 0)))
  (k-left-per-col (col ?y) (num ?ny&:(neq ?ny 0)))
  
  (or
    (guess-sure (x ?a1&:(eq ?a1 ?x)) (y ?b1&:(eq ?b1 (- ?y 1))))
    (guess-sure (x ?a2&:(eq ?a2 ?x)) (y ?b2&:(eq ?b2 (+ ?y 1))))
    (guess-sure (x ?a3&:(eq ?a3 (- ?x 1))) (y ?b3&:(eq ?b3 ?y )))
    (guess-sure (x ?a4&:(eq ?a4 (+ ?x 1))) (y ?b4&:(eq ?b4 ?y )))
  )
  (not 
    (exists (k-left-per-row (row ?x1) (num ?nx1))
      (exists (k-left-per-col (col ?y1) (num ?ny1))
        ...
        (or
          (guess-sure (x ?aa1&:(eq ?aa1 ?x1)) (y ?bb1&:(eq ?bb1 (- ?y1 1))))
          (guess-sure (x ?aa2&:(eq ?aa2 ?x1)) (y ?bb2&:(eq ?bb2 (+ ?y1 1))))
          (guess-sure (x ?aa3&:(eq ?aa3 (- ?x1 1))) (y ?bb3&:(eq ?bb3 ?y1 )))
          (guess-sure (x ?aa4&:(eq ?aa4 (+ ?x1 1))) (y ?bb4&:(eq ?bb4 ?y1 )))
        )
        (test (> (+ ?nx1 ?ny1) (+ ?nx ?ny)))
      )
    )
...
=>
  ...
  (assert (fired-gs (x ?x) (y ?y)))
  (pop-focus)
)



;; ####### NB --> Non faccio più tutte le 5 fire subito
;; NON Inizio il gioco usando tutte e 5 le fire.
...
(defrule max-fire (declare (salience 5))
  (status (step ?s) (currently running))
  (moves (fires ?nf &:(> ?nf 0)))
  (row-idx (xs $? ?x $?))
  (col-idx (ys $? ?y $?))
...
;; Per ogni REGOLE CERTE DI MOVIMENTO viene aggiunto una (assert (guess-sure (x) (y)))) con x e y coerenti
;; Ho una k-cell con contenuto left ==> metto una guess a destra
(defrule guess-right-sure (declare (salience 15))
  (status (step ?s) (currently running))
...
=>
  ...
  (assert (guess-sure (x ?x) (y (+ ?y 1))))
  (pop-focus)
)

;; Ho una k-cell con contenuto right ==> metto una guess a sinistra
(defrule guess-left-sure (declare (salience 15))
  (status (step ?s) (currently running))
...
(defrule guess-bot-sure (declare (salience 15))
  (status (step ?s) (currently running))
...
=>
  ...
  (assert (guess-sure (x (+ ?x 1)) (y ?y)))
  (pop-focus)
)

;; Ho una k-cell con contenuto bot ==> metto una guess sopra
(defrule guess-top-sure (declare (salience 15))
  (status (step ?s) (currently running))
...
(defrule guess-mid-ver-wall-bot-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (col-idx (ys $?ys))
...
(defrule guess-mid-ver-wall-top-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (col-idx (ys $?ys))
                                            ;; la max di una lista vuota non è definita
  (k-cell (x ?x) (y ?y&:(or (eq ?y 0) (eq ?y (max -1 (expand$ $?ys))))) (content middle)) ;; TODO: si può fare meglio?
...
(defrule guess-mid-hor-wall-right-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (row-idx (xs $?xs))
                                            ;; la max di una lista vuota non è definita
...
(defrule guess-mid-hor-wall-left-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (row-idx (xs $?xs))
                                            ;; la max di una lista vuota non è definita
  (k-cell (x ?x&:(or (eq ?x 0) (eq ?x (max -1 (expand$ $?xs))))) (y ?y) (content middle)) ;; TODO: si può fare meglio?
...
;; Ho una k-cell con contenuto mid e o la colonna a sinistra o la colonna a destra ha k-left-per-col 0
;; ==> metto una guess sotto
(defrule guess-mid-ver-top-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (k-cell (x ?x) (y ?y) (content middle))
  (or
    (k-left-per-col (col ?y1&:(eq ?y1 (- ?y 1))) (num 0))
...
;; ==> metto una guess a sinistra
(defrule guess-mid-hor-right-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (k-cell (x ?x) (y ?y) (content middle))
  (or
...
;; Ho una k-cell con contenuto mid e o la riga sopra o la riga sotto ha k-left-per-row 0
;; ==> metto una guess a sinistra
(defrule guess-mid-hor-left-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (k-cell (x ?x) (y ?y) (content middle))
  (or
    (k-left-per-row (row ?x1&:(eq ?x1 (- ?x 1))) (num 0))
...
(defrule guess-mid-mid-ver-bot-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (k-cell (x ?x) (y ?y) (content middle))
...
(defrule guess-mid-mid-hor-right-sure (declare (salience 15))
  (status (step ?s) (currently running))
  (k-cell (x ?x) (y ?y) (content middle))
